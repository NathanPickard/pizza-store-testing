describe('OrderService', function() {
  ...
  beforeEach(inject(function(_$httpBackend_, _$rootScope_, _OrderService_) {
    ...
    placeOrderHandler = $httpBackend.when('POST', '/api/order/place')
      .respond(200, {id: 3, status: 'in progress'});
    getStatusHandler = $httpBackend.when('GET', '/api/order/1/status')
      .respond(200, {status: 'not ordered'});
    getCouponHandler = $httpBackend.when('GET', '/api/order/coupon')
      .respond(200, {
        text: 'save 10% on your next order',
  ...
  describe('getOrderStatus', function() {
    it('returns null when order is not yet placed', function(done) {
    ...
    it('returns in progress after order is placed', function(done) {
  ...
  describe('getCoupon', function() {
    it('returns text and couponCode in response', function(done) {
      ...
      OrderService.getCoupon().then(function(coupon) {
        expect(coupon.text).toBe('save 10% on your next order');
